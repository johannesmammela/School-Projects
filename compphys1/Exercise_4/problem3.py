...
def cartesian_to_fractional(r, lattice):
    ...
    """
    return np.linalg.solve(lattice.T, r)   
...
def main():
    ...
    plt.figure()
    plt.plot(distances, dens_line)
    plt.xlabel("Distance along line (Å)")
    plt.ylabel("Electron density (Å^-3)")
    plt.title("Electron density along line")
    plt.tight_layout()
    #plt.show()
    plt.savefig("problem3b.pdf")
